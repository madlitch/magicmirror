 ...
 * MIT Licensed.
 */
Module.register("MMM-TouchButton-Unlock", {
  defaults: {
    animationSpeed: 0,
    classes: null,
    buttons: [],
  ...
  getScripts: function () {
    return [
      this.file("node_modules/jsonpath-plus/dist/index-browser-umd.js"),
      this.file("node_modules/js-uuid/js-uuid.js"),
      this.file("node_modules/@iconify/iconify/dist/iconify.min.js")
    ];
  },

  getStyles: function () {
    return ["font-awesome.css", "MMM-TouchButton-Unlock.css"];
  },
          ...
          if (curSource != null) {
            ...
            if (valResult) {
              ...
              if (typeof curCondition["classes"] !== "undefined") {
                curCondition["classes"]
                  .split(" ")
                  .forEach((element) => classes.push(element));
              }
              break;
            }
          }
  ...
  getDom: function () {
    ...
    if (self.config["classes"] != null) {
      self.config["classes"]
        .split(" ")
        .forEach((element) => moduleClasses.push(element));
    }
    ...
    for (let curId = 0; curId < self.config.buttons.length; curId++) {
      ...
      if (
        typeof curButtonConfig["profiles"] !== "undefined" &&
      ...
      let curCondButtonConfig = self.getCurrentButtonProps(
        curButtonConfig,
        self.results[curId] || {}
      );
      ...
      let curTitle = null;
      if (
        typeof curButtonConfig.title === "undefined" ||
      ...
      if (curTitle !== null) {
        ...
        curTitleObj.className =
          "touchButton button title title-" + curButtonConfig.name;
        curTitleObj.innerHTML = curTitle;
        curCondButtonConfig[2].forEach((element) =>
          curTitleObj.classList.add(element)
        );
      ...
      if (curCondButtonConfig[1] != null) {
        ...
        if (curCondButtonConfig[0].startsWith("fa ")) {
          ...
          curButtonIconWrapper.setAttribute(
            "data-icon",
            curCondButtonConfig[0]
          );
      ...
      if (curButton != null) {
        ...
        curCondButtonConfig[2].forEach((element) =>
          curButton.classList.add(element)
        );

        curButton.addEventListener("click", () => {
          self.sendSocketNotification("BUTTON_PRESSED", {
  ...
  start: function () {
    ...
    setTimeout(() => {
      for (
        let curNotiId = 0;
        curNotiId < self.config.notificationsAtStart.length;
        curNotiId++
      ) {
  ...
  notificationReceived: function (notification, payload) {
    ...
    if (self.watchNotifications) {
      if (typeof self.notifications[notification] !== "undefined") {
        ...
        for (
          let curCondBtnIdx = 0;
          curCondBtnIdx < curNotificationUsers.length;
          curCondBtnIdx++
        ) {
          ...
          if (curJsonpath != null) {
            ...
            if (curParsedPayload) {
              curResult = JSONPath.JSONPath({
                path: curJsonpath,
                json: curParsedPayload
              })[0];
            } else {
              curResult = payload;
            }
