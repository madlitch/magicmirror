...
Module.register("Medication-Alarm", {
  ...
  stopAlarm: function () {
    if (this.notificationReceivedTime) {/* global Module */
    ...
    Module.register("Medication-Alarm", {
      ...
      socketNotificationReceived: function (notification, payload) {
        if (notification === "MEDICATION_ALARM_TEST") {
          // Capture the current date and time when notification is received
          const notificationReceivedDateTime = new Date();
          
          // Send notification along with start date and time
          this.sendSocketNotification("MEDICATION_ALARM_TEST", {
            title: 'Medication Notification',
            message: `It's time to take ${payload.brand_name || payload.generic_name} at ${payload.time}`,
            startDateTime: notificationReceivedDateTime // Pass start date and time
          });
